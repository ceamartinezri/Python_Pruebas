...
#In this tutorial, we show how to use PennyLane to implement variational quantum classifiers - quantum circuits that can be trained from labelled data to classify new data samples
...
#We define the final model by a classical node that uses the first variable, and feeds the remainder into the quantum node. Before this, we reshape the list of remaining variables for easy use in the quantum node.
...
#…and train the optimizer. We track the accuracy - the share of correctly classified data samples.
# For this we compute the outputs of the variational classifier and turn them into predictions in  {-1,1} by taking the sign of the output

print("... now we track the accuracy ")
...
for it in range(25):
    ...
    t=threading.Thread(target=impresion_hilo)
    threads.append(t)
    #t.start()
...
#State preparation is not as simple as when we represent a bitstring with a basis state. Every input x has to be translated into a set of angles which can get fed into a small routine for state preparation.
#To simplify things a bit, we will work with data from the positive subspace, so that we can ignore signs (which would require another cascade of rotations around the z axis).
...
print("... now we print the angles and amplitude ")
...
#We then load the Iris data set. There is a bit of preprocessing to do in order to encode the inputs into the amplitudes of a quantum state.
#In the last preprocessing step, we translate the inputs x to rotation angles using the get_angles function we defined above.


data = np.loadtxt("variational_classifer\iris_classes1and2_scaled.txt")
X = data[:, 0:2]
...
#These angles are our new features, which is why we have renamed X to “features” above. Let’s plot the stages of preprocessing and play around with the dimensions (dim1, dim2).
#Some of them still separate the classes well, while others are less informative.
...
print ("Again we optimize the cost. This may take a little patience...") #PENDIENTE AGREGAR hilos para optimizar 
...
#Testing Optimization with threads
import threading
threads =list ()
...
def impresion_hilos (): 
    print(
        ...
        "".format(it + 0, cost(var, features, Y), acc_train, acc_val)
    )
...
for it in range(60):
    ...
    #Optimizing with threads
    t=threading.Thread(target=impresion_hilos)
    threads.append(t)
    #t.start()
